...
import com.example.kotlinlolapp.BuildConfig
...
class HttpClient {
    ...
    fun <T : Any>executeRequest(request: HttpRequest): HttpResponse<T> {
        ...
        return runCatching { sendRequest<T>(request) }.fold(
            ...
            onFailure = { throwable ->
                HttpResponse.Error(request.url, throwable).also { if (BuildConfig.DEBUG) Log.d(TAG, it.toString()) }
            }
        )
    }
    ...
    private fun <T : Any>retrieveResponse(request: HttpRequest, connection: HttpURLConnection): HttpResponse<T> {
        if (connection.responseCode !in 200 until 300) {
            return HttpResponse.Error(
                request.url,
                HttpException(connection.responseCode, connection.responseMessage.orEmpty()),
